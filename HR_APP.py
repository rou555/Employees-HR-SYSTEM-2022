...
class LoginScreen(QDialog):
    ...
    def loginfunction(self):
        ...
        self.user = self.ui.user_line.text()

        self.password = self.ui.pass_line.text()
...
class MainWin(QDialog):
    ...
    def __init__(self,userType):    
        ...
        if userType==2:
            ...
            self.ui.nuser_btn.setEnabled(False)
        else:
            pass

        self.ui.employee_btn.clicked.connect(self.gotoEmployeeMenu)
        self.ui.comp_btn.clicked.connect(self.gotoCompanyMenu)
    ...
    def gotoEmployeeMenu(self):
        EmpScreen = EmployeeMenu(self.usertype)
        widget.addWidget(EmpScreen)
        widget.setCurrentIndex(widget.currentIndex() + 1)
       
  
    def gotoCompanyMenu(self):
...
class EmployeeMenu(QDialog):
    ...
    def gotoeditemployee(self):
       EditEmployeescreen = EditEmployee(self.usertype)
...
class NewEmployee(QDialog):
    ...
    def SaveOnDataBase(self):
        ...
        if self.imgpath=="":
            ...
            print(self.imgpath)
            self.PhotoB=imageToBlob(self.imgpath)
...
class AddNewUser(QDialog):
    ...
    def save(self):
        ...
        user = self.ui.user_line.text()
        password = self.ui.pass_line.text()
        confirmpassword = self.ui.confirmpass_line.text()
        getUserType=self.ui.type_combo.currentText()

        if len(EmpId) ==0:
            QMessageBox.about(self, "alert","please Enter An Employee ID First") 
        if len(user)==0 or len(password)==0 or len(confirmpassword)==0:
            QMessageBox.about(self, "alert","please Fill The Missing Data First")
            print("error")
            ...
            IsFound=cur.fetchall()
            if IsFound == []:

                cur.execute("SELECT rowid FROM Employees WHERE Emp_ID = ?", (EmpId, )) # is the There is an Employee that exists 
                noEmployee = cur.fetchall()
                if noEmployee == []:
                    QMessageBox.about(self,"ERROR",'There is No Employee With This ID %s' % EmpId)
                else :
                    ...
                    user_info = [user,password,EmpId,userType]
                    cur.execute('INSERT INTO Users (User_Name, User_Password,Emp_ID,User_type) VALUES (?,?,?,?)', user_info)
                ...
                QMessageBox.about(self,"ERROR","Employee Has An Account")
    def goBack(self):
        back2wle = MainWin(self.usertype)
...
class CompanyInfo(QWidget):
    ...
    def goBack(self):
        back2wle = CompanyMeua(self.usertype)
        widget.addWidget(back2wle)
        widget.setCurrentIndex(widget.currentIndex()+1)
        widget.setWindowTitle("Main Screen")
